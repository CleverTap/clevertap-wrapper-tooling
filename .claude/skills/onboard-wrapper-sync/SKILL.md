...
- **Bash is denied outside the cwd, but Read/Grep/Glob tools are NOT.** Headless Claude's `Bash(grep:*)` etc. get permission-DENIED when the command references paths outside the wrapper checkout (e.g. the native source the diff tool cached under `~/.cache/clevertap-sdk-versions/`), even though the prefix is allowlisted. The Read/Grep/Glob TOOLS work on any absolute path. Prompts must say "use the Grep tool, not Bash" for native-source verification — a 2026-06-11 RN run guessed a selector after its Bash greps were denied and broke the build. (The PR-body prompt has the sync logs inlined for the same reason; don't switch it back to path-passing.)
- **Never put unverified native symbols in prompt reference code.** Claude copies reference implementations VERBATIM. The RN prompt's iOS example once called `[[CleverTap sharedInstance] fetchInbox]` — a selector that doesn't exist (only `fetchInboxWithCallback:` does) — and a sync shipped it straight into the bridge → post-sync build failure. Reference code must be source-verified when written, and carry a "verify, don't copy" warning.
