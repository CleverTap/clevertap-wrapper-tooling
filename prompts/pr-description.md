...
Render this section ONLY if `flagged_for_review` is non-empty in either platform's
sync log; otherwise omit it entirely. These are changelog-reported changes the
automation could not safely apply on its own — a human must decide. Keep it at the
TOP so it isn't missed. One bullet per item, prefixed by type:

- 🗑️ **Removal** / ⛔ **Deprecation** / 🔧 **Signature** / 🔁 **Behavior** / ❓ **Unconfirmed**: `<name>` (native v<changelog_version>) — <rationale>.
